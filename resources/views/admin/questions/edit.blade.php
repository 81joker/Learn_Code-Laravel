...
@section('content')
    @include('admin.users.partials.header', ['title' => __('Edit Quesions')])
                        ...
                        <div class="card-body">
                            ...
                            <form method="post" action="{{ route('questions.update' , $question) }}" autocomplete="off" enctype="multipart/form-data">
                                ...
                                <div class="pl-lg-4">
                                    <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                                        ...
                                        <input type="text" value="{{$question->title}}" name="title" id="input-title" class="form-control form-control-alternative{{ $errors->has('title') ? ' is-invalid' : '' }}" placeholder="{{ __('Quesions') }}" value="{{ old('title') }}" required autofocus >
                                    ...
                                    <div class="form-group{{ $errors->has('answers') ? ' has-danger' : '' }}">
                                       ...
                                       <input type="text" value="{{$question->answers}}" name="answers" id="input-answers" class="form-control form-control-alternative{{ $errors->has('answers') ? ' is-invalid' : '' }}" placeholder="{{ __('Answers') }}" value="{{ old('answers') }}" required>
                                    ...
                                    <div class="form-group{{ $errors->has('right_answer') ? ' has-danger' : '' }}">
                                        ...
                                        <textarea type="text" value="" name="right_answer" id="input-right_answer" class="form-control form-control-alternative{{ $errors->has('right_answer') ? ' is-invalid' : '' }}" placeholder="{{ __('Right_Answer') }}" value="{{ old('right_answer') }}"  required>
                            ...
                            <div class="form-group{{ $errors->has('quiz_id') ? ' has-danger' : '' }}">
                                ...
                                @foreach (\App\Quiz::all() as $quiz)
                                    <option value="{{$quiz->id}}" <?php if ($question->quit->id == $quiz->id ) {
                                        echo 'selected';
                                    }  ?> >{{$quiz->name}}</option>
                                @endforeach
                            </select>
