@extends('layouts.app', ['title' => __('Edit Courses ')])
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-title">{{ __('Title') }}</label>
                                     <input type="text" value="{{$course->title}}" name="title" id="input-title" class="form-control form-control-alternative{{ $errors->has('title') ? ' is-invalid' : '' }}" placeholder="{{ __('Title') }}" value="{{ old('title') }}" required autofocus>
                                ...
                                <div class="form-group{{ $errors->has('link') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" value="{{$course->link}}" name="link" id="input-link" class="form-control form-control-alternative{{ $errors->has('link') ? ' is-invalid' : '' }}" placeholder="{{ __('Link') }}" value="{{ old('link') }}" required>
                               ...
                               {{-- START STATUS --}}
                                <div class="form-group{{ $errors->has('status') ? ' has-danger' : '' }}">
                                    ...
                                    <select name="status" id="input-status" class="form-control" >
                                        <option value = "0" <?php if ($course->status == 0 ) {
                                            echo 'selected';
                                        }  ?> >Free</option>
                                        <option value= "1"<?php if ($course->status == 1 ) {
                                            echo 'selected';
                                        }  ?> >Bay</option>
                                    </select>
                                ...
                                <div class="form-group{{ $errors->has('track_id') ? ' has-danger' : '' }}">
                                    ...
                                    @foreach (\App\Track::all() as $track)
                                        <option value="{{$track->id}}"  <?php if ($course->track->id == $track->id) {
                                            echo 'selected';
